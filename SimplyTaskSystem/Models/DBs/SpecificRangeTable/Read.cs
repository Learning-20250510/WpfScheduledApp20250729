...
namespace SimplyTaskSystem.Models.DBs.SpecificRangeTable
{
    class Read
    {
        ...
        public ObservableCollection<DataClass> SelectSpecificRangeCollection(string commandtext)
        {
                ...
                using (var connection = new MySqlConnection(connectionString))
                {
                    using (var command = connection.CreateCommand())
                    {
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                PrioritiesCollection.Add(new DataClass { ID = int.Parse(reader["id"].ToString()), TasksTableID = int.Parse(reader["taskstable_id"].ToString()), Category = reader["category"].ToString(), CreatedAt = reader["created_at"].ToString() });
                            }
                        }
