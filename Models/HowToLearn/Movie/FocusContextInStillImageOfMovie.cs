...
{
    /// <summary>
    /// 動画の静止画（2秒間隔）にフォーカスしてコンテキストを学習
    /// </summary>
    public class FocusContextInStillImageOfMovie : IHowToLearnAction
    {
        ...
        public void TaskAction(string kmtName, string kmn, string htlName, int taskId)
        {
            ...
            // MMファイルを作成（静止画分析用）
            _freePlaneOperation.CreateBasicMMFile(kmtName, kmn, htlName + "_StillImage", taskId);
        }
